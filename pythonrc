...
print >> sys.stderr, 'Loading configuration file "~/.pythonrc".'
...
try:
  ...
  print >> sys.stderr, 'Warning: Module "readline" not available.'
else:
  import rlcompleter
  # Use C-t to trigger shell autocomplete, leaving Tab to insert literal tabs.
  readline.parse_and_bind("C-t: complete")
  # Remind myself that the mapping has been performed.
  print >> sys.stderr, 'Enabled key mapping: C-t -> autocomplete'
